...
internal static class CryingSessionImporter
{
    public static void Map(List<JsonCryingSession>? source, Dictionary<string, CryingSession> existingEntries, List<CryingSession> newEntries)
    {
        ...
        foreach (var item in source)
        {
            ...
            {
                throw new ImportValidationException(nameof(CryingSession), item.Pk, 
            ...
            if (existingEntries.TryGetValue(item.Pk, out var existing))
            {
                ...
                {
                    throw new ImportValidationException(nameof(CryingSession), item.Pk, ex.Message);
                }
            }
            else
            {
                try
                {
                    newEntries.Add(CryingSession.Create(
                        Guid.Empty,
                        item.Pk,
